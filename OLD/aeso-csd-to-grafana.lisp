...
;;;;
;;;; Fetches the AESO Current Supply & Demand (CSD) API and writes
;;;; metrics to InfluxDB (the most common Grafana time-series backend).
...
;;;;   usocket    – TCP sockets (used for MQTT and DNS error handling)
;;;;   cl+ssl     – TLS for AESO APIM
;;;;
;;;; Usage:
;;;;   sbcl --load aeso-csd-to-grafana.lisp
;;;;
;;;; The program can also be run as a polling loop; see *POLL-INTERVAL-SECONDS*.
...
(defun mqtt-publish (topic message)
  "Connect to the MQTT broker, publish MESSAGE to TOPIC, then disconnect.
   Uses a fresh TCP connection per publish (stateless/simple approach).
   Returns T on success, NIL on any error."
  (handler-case
      (let ((socket (usocket:socket-connect *mqtt-host* *mqtt-port*
        ...
        (unwind-protect
            (let ((stream (usocket:socket-stream socket)))
              ...
              (write-sequence (mqtt-connect-packet *mqtt-client-id*
                              ...
                              stream)
              (force-output stream)
              ;; Read CONNACK (4 bytes) — don't block forever
              (let ((connack (make-array 4 :element-type '(unsigned-byte 8))))
                (read-sequence connack stream)
                (unless (= (aref connack 3) 0)
                  (error "MQTT CONNACK refused, return code: ~A" (aref connack 3))))
              ;; PUBLISH
              (write-sequence (mqtt-publish-packet topic message) stream)
              (force-output stream)
              ;; DISCONNECT
              (write-sequence (mqtt-disconnect-packet) stream)
              (force-output stream)
              t)
          (usocket:socket-close socket)))
    (error (e)
      (format t "~&[MQTT] Publish failed (~A): ~A~%" topic e)
      nil)))
...
(defun mqtt-publish-poll (asset-count)
  "Publish a poll-success summary to <prefix>/poll."
  (let ((payload (mqtt-make-payload
...
(defun run-once ()
  "Fetch AESO CSD data once, push to InfluxDB, and publish MQTT notifications."
  (format t "~&[~A] Fetching AESO CSD...~%"
          (local-time:format-timestring nil (local-time:now)
                                        :format local-time:+iso-8601-format+))
  (let ((json (fetch-csd)))
    (if json
        (let* ((ts-ns  (now-unix-ns))
               (lines  (json->line-protocol json ts-ns)))
          (if lines
              (progn
                (format t "~&[INFO] Generated ~A line-protocol entries.~%" (length lines))
                (let ((write-ok (write-to-influx lines)))
                  (if write-ok
                      (progn
                        (setf *consecutive-failures* 0)
                        (mqtt-publish-poll (length lines)))
                      (progn
                        (mqtt-publish-error "influx_write_failed"
                                            "InfluxDB write returned an error")
                        (format t "~&[MQTT] Error notice published.~%")))))
              (progn
                (format t "~&[WARN] No line-protocol entries generated – check JSON structure.~%")
                (mqtt-publish-error "no_line_protocol" "JSON parsed but produced no line-protocol entries"))))
        (progn
          (format t "~&[WARN] No data returned from AESO API.~%")
          (mqtt-publish-error "fetch_failed" "AESO API returned no data")))))
